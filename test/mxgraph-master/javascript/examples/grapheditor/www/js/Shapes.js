...
(function()
{
	...
	{
		// 注意：GetValue不返回布尔值，所以！（'0'）将在此处和下方返回true
	...
	mxRhombus.prototype.paintVertexShape = function(c, x, y, w, h)
	{
		...
		if (!this.outline && this.style['double'] == 1)
		{
			...
			if (w > 0 && h > 0)
			{
				...
				// 解决闭包编译器bug的方法，其中x和y以上的行
				// 如果参数用作下面的调用中的第二个参数，则删除。
	...
	// 仅当定义了MxVertexHandler时才添加处理程序（即不在嵌入的图形中）
	if (typeof mxVertexHandler !== 'undefined')
	{
		function createHandle(state, keys, getPositionFn, setPositionFn, ignoreGrid, redrawEdges)
		{
			...
			if (redrawEdges)
			{
				...
				handle.positionChanged = function()
				{
					...
					// 所有的孩子redraws连通的边缘的边缘包括: Include child edges
		...
		var handleFactory = {
			...
			{
				// 不要使用state.shape.startsize/endsize，因为它是缓存的
		...
		mxVertexHandler.prototype.createCustomHandles = function()
		{
			...
			if (this.state.view.graph.getSelectionCount() == 1)
			{
				...
				// 稍后：使锁定状态独立于可旋转标志，如果默认值为false，则修复切换

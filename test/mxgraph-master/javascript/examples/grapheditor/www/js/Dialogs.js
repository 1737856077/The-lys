...
var ColorDialog = function(editorUi, color, apply, cancelFn)
{
	...
	// LATER: 只有当输入为焦点时才激发，如果显示此对话框，则应始终激发
	...
	var okBtn = mxUtils.button(mxResources.get('ok'), function()
	{
		...
		else if (select.value == 'replace')
		{
			...
			try
			{
				...
				// LATER: Why is hideDialog between begin-/endUpdate faster? 为什么begin-/endupdate之间的hidedialog更快？
				editorUi.hideDialog();
			}
			catch (e)
			{
...
/**
 * 获取导出格式的挂钩。返回默认中间XML导出格式的空值，或返回请求中要使用的参数和值的函数，格式为key=value，其中值应为url编码。
 */
 ...
 * 获取导出格式的挂钩。默认情况下返回空值中间XML导出格式或返回表单中要在请求中使用的参数和值key=value，其中value应该是url编码的。
 */
	...
	function refresh()
	{
		...
		function addLayer(index, label, child, defaultParent)
		{
			...
			mxEvent.addListener(ldiv, 'dragstart', function(evt)
			{
				...
				if (mxClient.IS_FF)
				{
					// LATER: 在ff-after-drop中检查是什么触发了对这个XML的解析

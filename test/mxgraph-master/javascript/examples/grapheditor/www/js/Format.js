...
Format.prototype.refresh = function()
{
	...
	else
	{
		...
		var addClickHandler = mxUtils.bind(this, function(elt, panel, index)
		{
			...
			{
				// 直接调用处理程序作为khtml中no click-on-div的解决方案。
				clickHandler();
			}
		});
...
BaseFormatPanel.prototype.installInputHandler = function(input, key, defaultValue, min, max, unit, textEditFallback, isFloat)
{
	...
	var update = mxUtils.bind(this, function(evt)
	{
		...
		// Special case: angle mod 360/特殊情况：角度模式360
		if (!isNaN(value) && key == mxConstants.STYLE_ROTATION)
		{
			// Workaround for decimal rounding errors in floats is to/浮点中小数舍入错误的解决方法是使用整数
			// use integer and round all numbers to two decimal point/并将所有数字舍入到小数点后两位
	...
	{
		// KNOWN: Arrow up/down clear selection text in quirks/IE 8/向上/向下箭头清除quirks/ie 8中的选择文本
		// Text size via arrow button limits to 16 in IE11. Why?/在IE11中，通过箭头按钮的文本大小限制为16。为什么？
...
BaseFormatPanel.prototype.createStepper = function(input, update, step, height, disableFocus, defaultValue)
{
	...
	// Disables transfer of focus to DIV but also :active CSS/禁用将焦点转移到DIV，但也禁用：活动CSS
	// so it's only used for fontSize where the focus should/所以它只用于fontsize，焦点应该是
	// stay on the selected text, but not for any other input./保留在所选文本上，但不用于任何其他输入
	if (disableFocus)
	{
...
BaseFormatPanel.prototype.createColorOption = function(label, getColorFn, setColorFn, defaultColor, listener, callbackFn, hideCheckbox)
{
	...
	var apply = function(color, disableUpdate, forceUpdate)
	{
		if (!applying)
		{
			...
			// Fine-tuning in Firefox, quirks mode and IE8 standards/在火狐、怪癖模式和IE8标准中进行微调
			if (mxClient.IS_QUIRKS || document.documentMode == 8)
			{
...
BaseFormatPanel.prototype.createCellColorOption = function(label, colorKey, defaultColor, callbackFn, setStyleFn)
{
	...
	}, defaultColor || mxConstants.NONE,
	{
		install: function(apply)
		{
			this.listener = function()
			{
				// Seems to be null sometimes, not sure why.../有时似乎是空的，不知道为什么…
				var state = graph.view.getState(graph.getSelectionCell());
...
TextFormatPanel.prototype.addFont = function(container)
{
	...
	// Workaround for offset in FF/FF中偏移的解决方法
	if (mxClient.IS_FF)
	{
	...
	// Hack for updating UI state below based on current text selection/基于当前文本选择更新以下用户界面状态的黑客
	// currentTable is the current selected DOM table updated below/当前表是在下面更新的当前选定的DOM表
	...
	// Adds label position options/添加标签位置选项
	...
	// Adds writing direction options/添加写入方向选项
	// LATER: Handle reselect of same option in all selects (change event/在所有选择中处理相同选项的重新选择（更改事件
	// is not fired for same option so have opened state on click) and/不是为同一选项激发的，因此单击时已打开状态）和
	// handle multiple different styles for current selection/处理当前所选内容的多种不同样式
	...
	// NOTE: For automatic we use the value null since automatic/对于automatic，我们使用值null，因为automatic
	// requires the text to be non formatted and non-wrapped/要求文本不格式化和不换行
	...
	if (!graph.isEditing())
	{
		...
		// LATER: 编辑时更新文本编辑器中的目录，并使用标签更新样式
		// NOTE: 棘手的部分是处理和传递自动值
	...
	// 如果未选择文本，则字体大小4的解决方法是更新以下字体大小
	// 在输入第一个字符之后（因为字体元素是惰性创建的）
	...
	function(fontSize)
	{
		// IE does not support containsNode/IE不支持containsNode
		// KNOWN: Fixes font size issues but bypasses undo/修复字体大小问题，但忽略撤消
			...
			if (par != null && isOrContains(graph.cellEditor.textarea, par))
			{
				...
				for (var i = 0; i < elts.length; i++)
				{
					if (elts[i].getAttribute('size') == '4')
					{
						...
						// 将输入中的fontsize替换为刚刚分配的解决方案
						// 对于不匹配的父元素的潜在fontsize值
	...
	else
	{
		...
		}, function(input)
		{
			...
			// KNOWN: 向上/向下箭头清除quirks/ie 8中的选择文本
			// 在IE11中，通过箭头按钮的文本大小限制为16。为什么？
			mxEvent.addListener(input, 'mousedown', function()
			{
				if (document.activeElement == graph.cellEditor.textarea)
				{
		...
		var btns = [
				...
				mxUtils.bind(this, function()
				{
					if (currentTable != null)
					{
						// Converts rgb(r,g,b) values/转换RGB（R、G、B）值
			...
			if (!updating)
			{
				...
				window.setTimeout(function()
				{
					...
					if (node != null)
					{
						...
						// Finds common font size/查找常用字体大小
						...
						if (css != null)
						{
							...
							// Workaround for firstChild is null or not an object/FirstChild的工作区为空或不是对象
							// in the log which seems to be IE8- only / 29.01.15/在日志中，似乎是IE8-仅限/29.01.15
...
StyleFormatPanel.prototype.addStroke = function(container)
{
	...
	mxEvent.addListener(styleSelect, 'change', function(evt)
	{
		...
		try
		{
			...
			// Default for rounded is 1/四舍五入的默认值为1
			var values = ['0', null];
	...
	// Used for mixed selection (vertices and edges)/用于混合选择（顶点和边）
	...
	function update(evt)
	{
		// Maximum stroke width is 999/最大笔划宽度为999
		var value = parseInt(input.value);
		value = Math.min(999, Math.max(1, (isNaN(value)) ? 1 : value));
	...
	var listener = mxUtils.bind(this, function(sender, evt, force)
	{
		...
		// Updates icon for edge shape/更新边缘形状的图标
		...
		// Special cases for markers/标记的特殊情况
		if (ss.style.shape == 'arrow')
		{
...
/**
 * Switch to disable page view./切换到禁用页面视图
 */
DiagramFormatPanel.showPageView = true;

/**
 * 指定是否应显示背景图像选项。默认值为true。
 */

 ...
 */
// 允许在HTML消毒剂中使用target=“_blank”的解决方案
...
// Matches label positions of mxGraph 1.x/匹配mxgraph 1.x的标签位置
 ...
 * in turn will create the container for the graph. Hence, the container is
 // 然后将为图表创建容器。因此，容器是
 * assigned later in EditorUi./后来在伊迪托里分配的。
 */
/**
 * Defines graph class.
 */
Graph = function(container, model, renderHint, stylesheet, themes)
{
	...
	if (this.edgeMode)
	{
		...
		this.addMouseListener(
		{
		    ...
		    {
		    	// Checks if any other handler is active/检查是否有其他处理程序处于活动状态
		    		...
		    		!mxEvent.isShiftDown(me.getEvent()) && !mxEvent.isAltDown(me.getEvent()))
		    	{
			    	...
			    	if (start.point != null && start.state != null && start.event != null)
			    	{
			    		...
			    		{
			    			// Lazy selection for edges inside groups/组内边缘的延迟选择
	    						...
	    						if (!entity || handle == 0 || handle == handler.bends.length - 1 || handle == mxEvent.LABEL_HANDLE)
	    						{
				    					...
				    					handle == handler.bends.length - 1 || state.visibleTargetState != null)
				    				{
				    					if (!entity && handle != mxEvent.LABEL_HANDLE)
				    					{
											...
											// 句柄位于角点句柄的默认情况
					    					// drag of corner as drag of existing point/拖动角作为现有点的拖动
					    						...
					    						edgeStyle == mxEdgeStyle.OrthConnector))
					    					{
					    						...
					    						if (handle == null)
					    						{
					    							...
					    							{
						    							// Moves source terminal handle/移动源终端句柄
					    								handle = 0;
					    							}
					    							else if (mxUtils.contains(box, pts[pts.length - 1].x, pts[pts.length - 1].y))
					    							{
					    								// Moves target terminal handle/移动源终端句柄
					    								handle = handler.bends.length - 1;
					    							}
					    							else
					    							{
							    						// Checks if edge has no bends/检查边缘是否没有弯曲
								    					...
								    					else
									    				{
									    					...
									    					// Maps segment to handle/映射要处理的段
									    					else
									    					{
									    						handle += 1;
									    					}
						    				...
						    				// Creates a new waypoint and starts moving it/创建新的航路点并开始移动它
						    				if (handle == null)
						    				{
				    					...
				    					// Removes preview rectangle in graph handler/删除图形处理程序中的预览矩形
				    					this.graphHandler.reset();
				    				}
	    						}
	    						else if (entity && (state.visibleSourceState != null || state.visibleTargetState != null))
	    						{
	    							// Disables moves on entity to make it consistent/禁用实体上的移动以使其一致
			    					this.graphHandler.reset();
	    							me.consume();
	    						}
			    			}
			    	...
			    	{
			    		// Updates cursor for unselected edges under the mouse/更新鼠标下未选定边的光标
				    	var state = me.getState();
	...
	if (typeof mxVertexHandler !== 'undefined')
	{
		...
		// Do not scroll after moving cells/移动单元格后不滚动
		this.graphHandler.scrollOnMove = false;
		this.graphHandler.scaleGrid = true;

		// Disables cloning of connection sources by default/默认情况下禁用连接源的克隆
		this.connectionHandler.setCreateTarget(false);
		this.connectionHandler.insertBeforeSource = true;
		
		// Disables built-in connection starts/禁用内置连接启动
		this.connectionHandler.isValidSource = function(cell, me)
		{
			return false;
		};

		// Sets the style to be used when an elbow edge is double clicked/设置双击肘形边缘时要使用的样式
		...
		// Adds page centers to the guides for moving cells/将页中心添加到移动单元格的参考线中
		...
		this.graphHandler.getGuideStates = function()
		{
			...
			// Create virtual cell state for page centers/为页中心创建虚拟单元状态
			if (this.graph.pageVisible)
			{
				...
				// Page center guides have predence over normal guides/页面中心参考线的前置性高于普通参考线

...
Toolbar.prototype.createTextToolbar = function()
{
	...
	// 已知：单击Quirks和IE8中带有文本（不是图标）的子菜单后，焦点丢失。这是因为TD似乎
	// 要获得焦点，请在这些浏览器中单击。注意：在MXPopupMenu中解决图标项（无文本）。
	...
	var formatMenu = this.addMenuFunction('', mxResources.get('format'), false, mxUtils.bind(this, function(menu)
	{
		...
		// 已知：IE+FF在颜色对话框后不返回键盘焦点（调用焦点没有帮助）
	...
	// Fixme:在这里使用gebutton，在主菜单中使用gelabel
...
/**
 * 将带有setter的启用状态添加到dom节点（避免使用JS包装器）。
 */

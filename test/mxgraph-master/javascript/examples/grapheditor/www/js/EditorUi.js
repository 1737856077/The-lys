...
EditorUi = function(editor, container, lightbox)
{
	...
	graph.panningHandler.isForcePanningEvent = function(me)
	{
		// 在Mac上的FF中，ctrl+left按钮报告为right按钮
	...
	// Ctrl/Cmd+Enter applies editing value except in Safari where Ctrl+Enter creates/ctrl/cmd+enter
	// 应用编辑值，但在safari中，ctrl+enter创建的值除外。
	// 新行（当Enter创建新段落和Shift+Enter停止时）
	...
	if (window.self === window.top && graph.container.parentNode != null)
	{
		...
		{
			// ignores error in old versions of IE/忽略旧版本IE中的错误
		}
	}
	...
	this.setDefaultStyle = function(cell)
	{
		...
		if (state != null)
		{
			...
			// Handles special case for value "none"/处理值“无”的特殊情况
	...
	// Adds all keys used above to the styles array/将上面使用的所有键添加到样式数组中
	...
	var insertHandler = function(cells, asText)
	{
		...
		try
		{
			// Applies only basic text styles/仅应用基本文本样式
			if (asText)
			{
			...
			else
			{
				for (var i = 0; i < cells.length; i++)
				{
					...
					// Applies the current style to the cell/将当前样式应用于单元格
					var edge = model.isEdge(cell);
					var current = (edge) ? graph.currentEdgeStyle : graph.currentVertexStyle;
	...
	this.addListener('styleChanged', mxUtils.bind(this, function(sender, evt)
	{
		...
		for (var i = 0; i < keys.length; i++)
		{
			...
			// Ignores transparent stroke colors/忽略透明笔划颜色
			if (keys[i] != 'strokeColor' || (values[i] != null && values[i] != 'none'))
			{
				// Special case: Edge style and shape/特殊情况：边缘样式和形状
				if (mxUtils.indexOf(connectStyles, keys[i]) >= 0)
				{
					...
					// Uses style for vertex if defined in styles/如果在样式中定义，则使用顶点样式
		...
		if (this.toolbar != null)
		{
			...
			{
				// Updates toolbar icon for edge style/更新边缘样式的工具栏图标
			...
			// Updates icon for optinal line start shape/更新可选线条开始形状的图标
			...
			// Updates icon for optinal line end shape/更新可选线条开始形状的图标
	...
	// Update font size and font family labels/更新字体大小和字体系列标签
	if (this.toolbar != null)
	{
	...
	// 在调整窗口大小或更改方向后更新编辑器用户界面
	// 对于旧的IE版本来说，超时是一种变通方法，因为它会延迟DOM客户机的大小
	// Should not use delay > 0 to avoid handle multiple repaints during window resize
	// 不应使用延迟>0以避免在窗口调整大小期间处理多个重绘。
	...
	// Workaround for bug on iOS see/iOS上的bug解决方法请参见
	...
	graph.addListener('gridSizeChanged', mxUtils.bind(this, function()
	{
   	...
   	// Resets UI, updates action and menu states/重置用户界面、更新操作和菜单状态
   	this.editor.resetGraph();
   	this.init();
   	this.open();
};

// Extends mxEventSource/扩展MxEventSource

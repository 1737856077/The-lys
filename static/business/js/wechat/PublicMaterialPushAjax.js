/**
 * Encoding     :   UTF-8
 * Created on   :   2015-08-07 15:54:00 by liuqingyan[Leaya] , liuqingyan0308@gmail.com
 * Id			:	PublicMaterialPushAjax.js
 * 描述			：	查询素材信息
 */
...
function WriteMaterialSearchHtml(objstr,page,type){
	...
	if(type=="image"){//图片
		...
		　　　　	tr.appendChild(td);
			var td=document.createElement("td");	
		　　　　	td.innerHTML='<a href="javascript:GetWechatMaterial(\''+obj[i]["MediaId"]+'\')">选中使用</a>';
		　　　　	tr.appendChild(td);
...
//		　　　　	tr.appendChild(td);
		...
		for (var i=0;i<obj.length;i++){ 
			var tr=document.createElement("tr");
				...
				tr.className="mid_02";
		　　
			if(obj[i]["Type"]=="atricles"){var _Type='单图文'; }else{var _Type='多图文'; }
			var td=document.createElement("td");	
		　　　　	td.innerHTML=''+obj[i]["Title"]+'（'+_Type+'）';
		　　　　	tr.appendChild(td);
			var td=document.createElement("td");	
		　　　	td.innerHTML='<a href="javascript:GetWechatMaterial(\''+obj[i]["MediaId"]+'\')">选中使用</a>';
		　　　　	tr.appendChild(td);

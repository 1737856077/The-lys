...
function WriteMaterialSearchHtml(objstr,page,type){
	...
	if(type=="image"){//图片
		...
		　　　　	tr.appendChild(td);
			var td=document.createElement("td");	
		　　　　	td.innerHTML='<a href="javascript:GetWechatMaterialImage(\''+obj[i]["Title"]+'\',\''+obj[i]['MediaId']+'\')">选中使用</a>';
		　　　　	tr.appendChild(td);
...
function GetWechatMaterialImage(title,mid){
	document.getElementById("thumb_media_id").value=mid;
    document.getElementById("thumb_media_title").value=title;
	Oclick_claseDialogBtn();
}
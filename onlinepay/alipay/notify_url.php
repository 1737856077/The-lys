...
if($result) {//验证成功
    ...
    //file_put_contents ( dirname ( __FILE__ ).DIRECTORY_SEPARATOR."log.txt", date ( "Y-m-d H:i:s" ) . "  " . json_encode($_POST) . "\r\n", FILE_APPEND );
    $DB=new DB();
    $gettime=time();
    ...
    //else if ($_POST['trade_status'] == 'TRADE_SUCCESS') {
        ...
        $getoneOrder=$DB->getone("SELECT * from sy_order where order_no = '$out_trade_no'");

        if (!empty($getoneOrder))
        {
            if($getoneOrder['score_real_pay'] != $total_amount){
                //echo "付款金额错误，请核对，需付款金额：".$getoneOrder['score_real_pay']."元，当前付款金额：".$total_amount;
                file_put_contents ( dirname ( __FILE__ ).DIRECTORY_SEPARATOR."log.txt", date ( "Y-m-d H:i:s" ) . "  " . "付款金额错误，请核对，需付款金额：".$getoneOrder['score_real_pay']."元，当前付款金额：".$total_amount. "\r\n", FILE_APPEND );

 ...
 * $Id:functions.php 2015-05-18 11:25:00 $
 */
 ...
 * 车牌遮挡处理程序
 */
function license($license, $replace = "*", $start = 4, $length=3){
    $str = '';
    for($i = 0;$i < $length; $i++){
        $str .= $replace;
    }
 ...
 */
function returnWeekStr($n,$type=0){
...
function wirtefile($contnet){
	$file = fopen('/runtime/temp/Log/'.date("Ymd").".txt", 'a+');
 ...
 * @return 	string				--日期描述
 */
function ReturnDateString($time,$type=0)
{
	//获取今天凌晨的时间戳
